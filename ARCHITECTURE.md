...
- **Transcript revision model**: `TranscriptRevision` (`confirmedText` append-only, `interimText` full replacement snapshot, optional `appendSafeSuffix` for append-only providers) is the provider-neutral seam. `TranscriptRevisionBuffer` turns revisions into cursor output operations (`erase` / `type`), keeping typed text consistent when interim hypotheses revise earlier text; output layers never see provider protocol details. `WhisperNativeAgent` emits these operations through `onOutputOps` and delivers the final transcript exactly once.
