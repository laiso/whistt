...
Internal architecture of the Whistt push-to-talk app for macOS, built on the OpenAI Realtime, Google Gemini Live, Meta Muse Voice Transcribe, xAI streaming Speech-to-Text, and Microsoft Azure Voice Live WebSocket APIs.
...
- **API key storage**: The app uses `KeychainStore` (generic-password, service `so.lai.whistt`) with separate `OPENAI_API_KEY`, `GEMINI_API_KEY`, `META_API_KEY`, `XAI_API_KEY`, and `AZURE_SPEECH_API_KEY` accounts. Lookup order is process environment > Keychain > legacy `.env` migration > provider-specific prompt. Azure additionally requires a Voice Live endpoint; it is not a secret, so `AzureVoiceLiveSettings` stores it in `UserDefaults` (`WHISTT_AZURE_SPEECH_ENDPOINT`), normalizing trailing slashes and requiring an `https://` URL with a host. Resolution order for the endpoint is process environment / `.env` > stored value. When either Azure credential is missing, the app opens the Provider Settings window instead of the key-only prompt. CLI probes use `.env` through `EnvLoader` because unsigned CLIs cannot share the app's Keychain entry.
...
- **Provider-specific audio**: OpenAI receives PCM16/24kHz/mono. Gemini receives little-endian PCM16/16kHz/mono with `audio/pcm;rate=16000`. Meta receives binary PCM16LE/24kHz/mono frames paced by the live microphone. xAI receives PCM16LE/16kHz/mono frames in 100ms chunks. Azure Voice Live receives base64-encoded PCM16/24kHz/mono in 100ms JSON appends. The converter target follows the selected transport.
...
- **Transcript revision model**: `TranscriptRevision` (`confirmedText` append-only, `interimText` full replacement snapshot, optional `appendSafeSuffix` for append-only providers) is the provider-neutral seam. `TranscriptRevisionBuffer` tracks provider revisions internally and can turn them into cursor output operations (`erase` / `type`); output layers never see provider protocol details. xAI currently suppresses those operations and delivers only the final transcript once because revising text in another app cannot be rolled back safely after a cancelled capture.
- **xAI push-to-talk**: `XaiTranscriptionTransport` connects to `wss://api.x.ai/v1/stt?encoding=pcm&sample_rate=16000&interim_results=true` with `Authorization: Bearer`, waits for `transcript.created` before sending audio, and maps `transcript.partial` by `is_final`/`speech_final` onto the revision model. Chunk-final snapshots are cumulative, so the delivered suffix is a prefix diff; `speech_final=true` emits the utterance final and `turnComplete`. `endInput` sends `audio.done`, after which the server sends `transcript.done` and closes the socket — that teardown is treated as success, and any other close or error returns the app to a recoverable state via `onError`. Language is left to auto-detection.
- **Azure Voice Live push-to-talk**: `AzureVoiceLiveTransport` connects to `<AZURE_SPEECH_ENDPOINT>/voice-live/realtime?api-version=2026-04-10&model=gpt-4.1` with an `api-key` header. The URL `model=` selects the hosted conversation model; the transcription model (`mai-transcribe-2`) goes into `session.input_audio_transcription.model` via `session.update` sent on socket open, together with `azure_semantic_vad_multilingual` server VAD (`create_response: false`). Audio is gated until `session.updated` (→ `ready`). `endInput` sends `input_audio_buffer.commit`; VAD normally commits the turn on its own. `input_audio_buffer.committed` maps to `turnComplete` and `conversation.item.input_audio_transcription.completed` to `final`. Verified live: mai-transcribe-2 emits no interim results over Voice Live, so only the final transcript reaches the output sink and language is left to auto-detection. Voice Live and mai-transcribe-2 are preview features; unknown events are surfaced as `.unknown` diagnostics rather than dropped, and protocol errors close the socket so the app can retry.
