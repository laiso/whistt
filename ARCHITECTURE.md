...
- **Azure Voice Live push-to-talk**: `AzureVoiceLiveTransport` connects to `<AZURE_SPEECH_ENDPOINT>/voice-live/realtime?api-version=2026-04-10&model=gpt-4.1` with an `api-key` header. The URL `model=` selects the hosted conversation model; the transcription model (`mai-transcribe-2`) goes into `session.input_audio_transcription.model` via `session.update` sent on socket open. Automatic turn detection is disabled so one hotkey press produces one transcript even when speech contains pauses. Audio is gated until `session.updated` (→ `ready`), and `endInput` sends `input_audio_buffer.commit`. `input_audio_buffer.committed` maps to `turnComplete` and `conversation.item.input_audio_transcription.completed` to `final`. Verified live: mai-transcribe-2 emits no interim results over Voice Live, so only the final transcript reaches the output sink and language is left to auto-detection. Voice Live and mai-transcribe-2 are preview features; unknown events are surfaced as `.unknown` diagnostics rather than dropped, and protocol errors close the socket so the app can retry.
