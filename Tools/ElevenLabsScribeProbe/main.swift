...
guard CommandLine.arguments.count == 2 else {
    FileHandle.standardError.write("usage: elevenlabs-scribe-probe /path/to/pcm16le-16khz-mono.raw\n".data(using: .utf8)!)
    exit(2)
}

guard let apiKey = EnvLoader.value(for: "ELEVENLABS_API_KEY") else {
    FileHandle.standardError.write("ELEVENLABS_API_KEY is required\n".data(using: .utf8)!)
    exit(2)
}
...
let socket = ElevenLabsScribeTransport(
    apiKey: apiKey,
    model: ElevenLabsScribeMessage.defaultModel
)
